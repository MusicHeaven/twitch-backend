...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
            // prevent jdbc error (required by official doc)
            // similar to Driver driver = new Driver(), but it needs to have the Driver class loaded in compile time
            // line 18 don't need to have "com.mysql.cj.jdbc.Driver" loaded when compile, and load it in runtime. (reflection)
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();
            // final can assign value once
    ...
    public void close() {
        if (conn != null) {
            try {
                // one database may only allow 50 connection, so need to close and others can access
                conn.close();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // check whether the item exist in item table. insert if not exist
        // or put all the item fro Twitch into Item table.
        // and favorite must login first, if can login , then user must exist. So don't have to save(user)
        saveItem(item);
        // ignore: if the data inserting exists, just ignore, without throw exception.
        String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (?, ?)";
        try {
            // preparestatement is developed by sql, and will exeamine the statement before execure.
            // is statement says WHERE "id = 1 or 1 = 1", the string template will view think 1 = 1 is true, and WHERE donot make sense
            // but preparestatement will view "id = 1 or 1 = 1" as one string
            // same for  WHERE "id = 1, drop table", string template will execure before  WHERE "id = 1, and drop table.
            // preparestatement will not.
    ...
    public void unsetFavoriteItem(String userId, String itemId) throws MySQLException {
        // dont need to delete from Item table everytime, can same a bulk and delete, reduce latency
        if (conn == null) {
            System.err.println("DB connection failed");
            throw new MySQLException("Failed to connect to Database");
        }
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
            // executeQuery used in execute read statement
            // ResultSet is a table, by default rs pointing to index -1, like iterator, use .next to iterate

            // if ResultSet has 100 columns, can use ORM to convert table to convert to java object
            // similar to ORM can convert JSON to JAVA Object and JAVA Object to JSON
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            // check whether successfully insert 1 line
        } catch (SQLException e) {
            e.printStackTrace();
